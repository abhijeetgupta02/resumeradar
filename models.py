...
class OpenRouterProvider:
    ...
    proxies to many underlying models (Gemini, Llama, DeepSeek, Qwen, etc.).
    We talk to it with `requests` (already a project dependency) so no extra
    package is required. Responses are adapted to the Ollama-like shape
    `{"message": {"role": ..., "content": ...}}` used everywhere else.
    """
    ...
    def chat(
        ...
        **kwargs
        ...
        # When a structured `format` (JSON schema) is requested by the caller,
        # nudge the model toward valid JSON. Not every model supports this, so
        # we transparently retry without it if the API rejects the request.
        ...
        headers = {
            ...
            "HTTP-Referer": "https://github.com/interviewstreet/hiring-agent",
        ...
        MAX_DELAY = 60.0  # cap so we never wait more than 1 minute
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                # Drop it once and retry rather than failing outright.
                if (
                ...
                # OpenRouter returns an OpenAI-style payload. Surface API-level
                # errors (which can arrive with a 200) clearly.
                ...
                sleep_time = round(delay * random.uniform(0.8, 1.2), 2)
                print(
                    f"[OpenRouterProvider] Request failed: {e} "
                    f"(attempt {attempt + 1}/{MAX_RETRIES}). "
                    f"Retrying in {sleep_time}s..."
                )
                time.sleep(sleep_time)
