...
def main(pdf_path):
    ...
    # Return the evaluation plus the in-memory context the UI needs to render
    # rich results (profile, projects, parsed sections) WITHOUT reading disk
    # cache — so the public deployment can run with caching/PII storage off.
    return {
        "evaluation": score,
        "resume_data": resume_data,
        "github_data": github_data,
